...
<body>
<div id="container">
	<h1>[${funding.region }]&nbsp;&nbsp;${funding.product_name }</h1> <!-- 상품이름 뜨게 -->
	<hr>
<div>
...
<table class="reward">
	<tr>
		...
		<c:forEach var="name_price_map" varStatus="status" items="${name_price_map}">
			...
			<input type="checkbox" value="${name_price_map.value}"
			 id="${name_price_map.key }" name="reward${status.index }" class="opt" onclick="sum(this)">
	...
	<!-- 수정/삭제-->
	<%--  	<c:if test="${login == null || login != funding.id}">
     ...
     --%>
       <%-- <c:if test="${login == funding.id}"> --%>
			...
			<%-- <a href="deleteBoard.do?product_no=${funding.product_no}">삭제</a>--%>
		<%--</c:if> --%>
</div>
</body>
</html>
